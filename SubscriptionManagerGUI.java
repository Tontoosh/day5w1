...
public class SubscriptionManagerGUI extends JFrame {
    ...
    public SubscriptionManagerGUI() {
        List<Subscription> initialSubs = List.of(
                new ProductSubscription("S0001", "Бат Энх",    "99119911", LocalDate.parse("09-25-2025", dateFormatter), "$35.00", "Сар бүр", "Явж байна"),
                new ServiceSubscription("S0002", "Эрхэс Бат",  "99112233", LocalDate.parse("09-25-2025", dateFormatter), "$35.00", "Сар бүр", "Явж байна")
        );
    ...
    private void showForm(Subscription existing) {
        ...
        while (true) {
            ...
            if (!phoneVal.matches("\\d{8}")) {
                JOptionPane.showMessageDialog(this, "Утасны дугаарыг зөв оруулна уу (8 оронтой).", "Алдаа", JOptionPane.ERROR_MESSAGE);
                continue;
            }
            if (!recurringVal.matches("\\$\\d+(\\.\\d{2})?")) {
                JOptionPane.showMessageDialog(this, "Дахин төлбөрийг зөв оруулна уу (жишээ: $35.00).", "Алдаа", JOptionPane.ERROR_MESSAGE);
                continue;
            }
            LocalDate nextDateObj;
            try {
                nextDateObj = LocalDate.parse(nextDateVal, dateFormatter);
